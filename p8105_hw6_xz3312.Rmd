...
glance(mod_main)
```
The chosen model explains a substantial proportion of the variability in 
birthweight, with an \(R^2\) of `r round(glance(mod_main)$r.squared, 3)`. 
Most predictors, including baby length, head circumference, gestational age, 
maternal BMI, and weight gain, show strong associations with birthweight, 
while smoking is associated with lower birthweight. The residual standard 
error is approximately `r round(glance(mod_main)$sigma, 1)` grams.
